...
```bash
pyinstaller --onefile --noconsole --uac-admin --noupx --runtime-tmpdir "C:\Temp\ok_kes" --name "卡厄思自动化工具v1.2.9" --icon icons/icon.ico ^
...
```bash
rmdir /s /q build
del "卡厄思自动化工具v1.2.9.spec"
```

### 4. 产物

位于 `dist\卡厄思自动化工具v1.2.9.exe`，约 265 MB。
