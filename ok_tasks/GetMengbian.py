...
class GetMengbian(BaseTask):
    ...
    def run(self):
        for i in range(300):
            ...
            self.log_info("梦之边境页面触发退出事件，点击退出位置")
            self.click(0.959, 0.049)
            self.log_info("梦之边境退出页面触发逃脱事件，等待并点击「逃脱」")
            self.wait_click_ocr(match="逃脱", time_out=60)
            self.log_info("梦之边境退出页面触发确认事件，等待并点击「确认」")
